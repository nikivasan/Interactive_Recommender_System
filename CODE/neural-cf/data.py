...
class SampleGenerator(object):
    ...
    def __init__(self, ratings, is_explicit=True):
        """
        args:
        ...
        self.max_rating = ratings['rating'].max() if is_explicit else 1.0
        SampleGenerator.max_rating = self.max_rating
    ...
    def _sample_negative(self, ratings):
        ...
        if self.is_explicit:
            raise ValueError("Negative sampling is not supported for explicit feedback")
    ...
    def evaluate_data(self):
        ...
        if self.is_explicit:
            return [
                torch.LongTensor(self.test_ratings['userId'].tolist()),
                torch.LongTensor(self.test_ratings['itemId'].tolist()),
                torch.FloatTensor(self.test_ratings['rating'].tolist())
            ]
        else:
